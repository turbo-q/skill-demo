...
- **Middleware 架构**：使用 LangChain `create_agent` + middleware 链（TodoList, Skills, Summarization）
...
当前有两个示例技能（均由 SkillsMiddleware 自动加载）：
...
- 新会话或选已有会话，在输入框发送与技能相关的自然语言，例如：
  ...
  - 测 **web-basic-scan**：`对 http://example.com 做一次基础 Web 扫描`
- Agent 会按上下文自动注入对应技能说明，并调用绑定工具（如 `tcp_port_scan`、`http_get`）执行。
...
- `app/agent_vuln.py`：Agent 构造函数（`get_agent()`），使用 LangChain `create_agent` + middleware 链（TodoList, Skills, Summarization）+ LangGraph checkpoint。
